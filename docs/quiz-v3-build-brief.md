...
| Screen 31 poll and `/manage` | `POST /web-profile-status` `{ rythma_id }` → `{ paid, plan, trial_ends_at, code, manage_url }` (ruled 2026-09-14; the app session is adding it). Replace the per-process memory bridge with this; keep the 30-second "it's in your email" fallback. Mock it like the other two until `SP_PROFILE_API_URL` is set. |
